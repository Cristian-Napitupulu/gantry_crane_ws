...
def fuzzytype2(u, e, tet, u0=575, mode="trolley"):
    domain1 = linspace(0.0, 5.0, 100)
    Z1 = IT2FS_Gaussian_UncertStd(domain1, [0, 0.4, 0.2, 1.0])
    S1 = IT2FS_Gaussian_UncertStd(domain1, [1.5, 0.3, 0.2, 1.0])
    B1 = IT2FS_Gaussian_UncertStd(domain1, [5, 1.5, 0.3, 1.0])

    domain2 = linspace(0, 1, 100)
    # Z2 = IT2FS_Gaussian_UncertStd(domain2, [0, 0.003, 0.001, 1.])
    Z2 = IT2FS_Gaussian_UncertStd(domain2, [0, 0.003, 0.001, 1.0])
    S2 = IT2FS_Gaussian_UncertStd(domain2, [0.4, 0.2, 0.1, 1.0])
    B2 = IT2FS_Gaussian_UncertStd(domain2, [1, 0.2, 0.1, 1.0])
    ...
    myIT2FLS.add_rule([("u", B1)], [("un", {"const": u0, "u": cu, "e": 0.0, "tet": 0})])
    myIT2FLS.add_rule([("u", S1)], [("un", {"const": u0, "u": cu, "e": 0.0, "tet": 0})])
    myIT2FLS.add_rule(
        [("u", Z1), ("e", S2)], [("un", {"const": u0, "u": cu, "e": 0.0, "tet": 0})]
    )
    myIT2FLS.add_rule(
        [("u", Z1), ("e", B2)], [("un", {"const": u0, "u": cu, "e": 0.0, "tet": 0})]
    )
    myIT2FLS.add_rule(
        [("u", Z1), ("tet", B3)], [("un", {"const": u0, "u": cu, "e": 0.0, "tet": 0})]
    )
    myIT2FLS.add_rule(
        [("u", Z1), ("e", Z2), ("tet", Z3)],
        [("un", {"const": 0, "u": 0.0, "e": 0.0, "tet": 0})],
    )
...
class Controller:
    ...
    def get_control_signal(self, desired_trolley_position, desired_cable_length=None):
        ...
        if desired_cable_length is None:
            desired_cable_length_ = self.connector.variables_value["cable_length"]
        ...
        print(f"Control_L: {CS2},e_l: {self.el}")
        ...
        print(
            "Control now: {}, {}.".format(
                self.control_signal_trolley, self.control_signal_hoist
            )
        )
        if desired_cable_length is None:
            self.control_signal_hoist = 0.0
    ...
    def convert_to_pwm(self, trolley_control_input, hoist_control_input=None):
        ...
        ux0 = 880
        ufuzzx = fuzzytype2(
            trolley_control_input, self.ex, tetha, ux0, "trolley"
        )  # 630
...
if __name__ == "__main__":
    ...
    try:
        ...
        result = control_gantry_crane(
            ...
            desired_trolley_position=1.250,
