...
MODEL_PATH = "/home/icodes/Documents/Shabri_13320075/TugasAkhir/Programs/Models/Matured/Model tanpa l.pt"
CONTROL_TYPE = 1
...
class Controller:
    ...
    def Hamiltonian(self, Xf):
        ...
        ptet = Xf[2]
        px = Xf[3]
        l=torch.tensor(0.4).to(self.device)
        ...
        M0 = torch.cat([M00, M01])
        M1 = torch.cat([M10, M11])
        ...
        ptet = ptet.reshape(1)
        px = px.reshape(1)
        p = torch.cat([ptet, px], 0)
        p = torch.reshape(p, (1, 2))
        ...
        Hm = self.hlf * torch.matmul(
            ...
            0.4 * (self.uni - torch.cos(tet)) + torch.tensor(0.7) - 0.4
        )
    ...
    def get_parameter(self, parameter_json_path):
        ...
        self.R = torch.zeros((4, 4)).float().to(self.device)
        self.R[3:3] = DAMPING_TETA
        self.R[4:4] = DAMPING_X

        self.J = torch.zeros((4,4), dtype=torch.float32).to(self.device)
        self.J[0, 2] = torch.Tensor([1.0])
        self.J[1, 3] = torch.Tensor([1.0])
        self.J[2, 0] = torch.Tensor([-1.0])
        self.J[3, 1] = torch.Tensor([-1.0])
    ...
    def get_control_signal(self, desired_trolley_position, desired_cable_length=None):
        ...
        desiredStateVector = torch.tensor([desired_trolley_position]).to(self.device)
        ...
        Hamiltonian_plant = self.Hamiltonian(neural_networks_input[0:4])

        virtual_potential = torch.square(
            neural_networks_input[1] - neural_networks_input[4]
        )
        ...
        control_signal = torch.matmul(
            ...
            gradient_of_hamiltonian_desired[0:4],
        ) - torch.matmul((self.J - self.R), gradient_of_hamiltonian_plant[0:4])
